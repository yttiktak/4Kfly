#ifndef MYRP_LITTEXARRAY_INCLUDED
#define MYRP_LITTEXARRAY_INCLUDED
...
VertexOutput LitTexArrayPassVertex (VertexInput input) {
	VertexOutput output;
...
float4 LitTexArrayPassFragment (VertexOutput input) : SV_TARGET {
	UNITY_SETUP_INSTANCE_ID(input);
	input.normal = normalize(input.normal);
	float3 albedo = UNITY_ACCESS_INSTANCED_PROP(PerInstance, _Color).rgb;
	float3 texcolor = _MainTexA.Sample(sampler_MainTexA, input.uv);
...
#endif // MYRP_LITTEXARRAY_INCLUDED