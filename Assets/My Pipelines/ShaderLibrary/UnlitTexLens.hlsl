#ifndef MYRP_UNLITYEXLENS_INCLUDED
#define MYRP_UNLITYEXLENS_INCLUDED
...
float4 UnlitTexArrayPassFragment (VertexOutput input) : SV_TARGET {
	UNITY_SETUP_INSTANCE_ID(input);
	    ...
	    rv = input.uv4 + (input.uv3 - input.uv4) * _mag;

		texcolor = _MainTex.Sample(sampler_MainTex, rv);

	return float4( texcolor, 1);
}

#endif // MYRP_UNLITYEXLENS_INCLUDED
