#ifndef MYRP_LITVERTEXCOLORS_INCLUDED
#define MYRP_LITVERTEXCOLORS_INCLUDED
...
float4 _neutral_color;
float _reactive;

VertexOutput LitVertexColorsPassVertex (VertexInput input) {
	VertexOutput output;
	...
	output.worldPos = worldPos.xyz;
	        	...
	        	float factor = _reactive;
                output.clipPos.xyz += input.normal * factor * _hydrophobic_color.w * dot(input.color-_neutral_color,_hydrophobic_color);
...
float4 LitVertexColorsPassFragment (VertexOutput input) : SV_TARGET {
	UNITY_SETUP_INSTANCE_ID(input);
	input.normal = normalize(input.normal);
	float3 albedo = input.color; // UNITY_ACCESS_INSTANCED_PROP(PerInstance, _Color).rgb;
