#ifndef MYRP_LITTEXGEOM_INCLUDED
#define MYRP_LITTEXGEOM_INCLUDED
...
struct GSPS_Input {
	...
	float3 uv : TEXCOORD3; // why are you using texcoord0 for naormals?
...
[maxvertexcount(3)]
void LitTexGeomPassGeometry( triangle GSPS_Input input[3],inout TriangleStream<GSPS_Input> TriStream) 
	{
		GSPS_Input output;
		for (uint i = 0; i < 3; i++) {
			...
			output.vertexLighting = input[i].vertexLighting;
...
VertexOutput LitTexGeomPassVertex(VertexInput input) {
	VertexOutput output;
...
float4 LitTexGeomPassFragment (VertexOutput input) : SV_TARGET {
	UNITY_SETUP_INSTANCE_ID(input);
	...
	float3 color = _emissivity * texcolor + diffuseLight * albedo  * texcolor; 
	// float3 color = diffuseLight * albedo;

	return float4(color, 1);
}

#endif // MYRP_LITTEXGEOM_INCLUDED